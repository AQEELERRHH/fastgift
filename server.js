...
app.post('/api/quote', async (req, res) => {
  try {
    const fetch = await getFetch();
    const r = await fetch(BASE + '/quote', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(req.body) });
    res.json(await r.json());
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/order', async (req, res) => {
  try {
    const fetch = await getFetch();
    const { x402Pay } = await import('@fastxyz/x402-client');
    const { quote_id } = req.body;
    const wallet = { type: 'fast', privateKey: PRIVATE_KEY, publicKey: '', address: FAST_ADDRESS };
    const response = await x402Pay(
      () => fetch(BASE + '/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ quote_id }) }),
      { wallet }
    );
    const text = await response.text();
    console.log('Order response:', text.slice(0, 200));
    try { res.json(JSON.parse(text)); }
    catch(e) { res.json({ error: 'Unexpected response', raw: text.slice(0, 200) }); }
  } catch (err) {
...
app.get('/api/order/:id', async (req, res) => {
  try {
    ...
    const r = await fetch(BASE + '/orders/' + req.params.id, { headers: { 'Authorization': 'Bearer ' + auth } });
...
app.listen(PORT, () => console.log('FastGift running on port ' + PORT));
